...
class ChatRendererFactory:
    ...
    def get_renderer(
        ...
        if model_template_type:
            ...
            if not renderer and model_template_type in _renderer_factory:
                logging.info(
                    f"Renderer factory found MODEL_TEMPLATE_TYPE [{model_template_type}] in dedicated renderer factory, use this."
                )
                renderer = _renderer_factory[model_template_type](
                    tokenizer,
                    new_params,
                )
        ...
        if not renderer and params.model_type in _renderer_factory:
            logging.info(
                f"Renderer factory found model type [{params.model_type}] has dedicated renderer, use this."
            )
            renderer = _renderer_factory[params.model_type](tokenizer, params)
        ...
        if not renderer:
            try:
                if tokenizer.chat_template != None:
                    ...
                    renderer = BasicRenderer(tokenizer, params)
                else:
                    pass
        ...
        if not renderer:
            ...
            if imported_template_renderer:
                ...
                renderer = imported_template_renderer
        ...
        if getattr(params, "custom_modal_config", None):
            logging.info(f"Wrapping renderer {type(renderer).__name__} with CustomModalProxyRenderer for custom modal support.")
            renderer = CustomModalProxyRenderer(tokenizer, params, renderer)
